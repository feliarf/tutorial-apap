...
public class StoreControllerTest {
    ...
    @Test
    public void whenRoutingViewCalledItShouldReturnModelAttr() throws Exception {
        StoreModel newStore = generateDummyProductModel(1);
        when(storeService.getStoreById(1L)).thenReturn(newStore);
        mockMvc.perform(get("/store/view?idStore=1"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(content().string(Matchers.containsString("Informasi Store")))
                .andExpect(content().string(Matchers.containsString("Daftar Produk:")))
                .andExpect(model().attribute("store", hasProperty("id", is(1L))))
                .andExpect(model().attribute("store", hasProperty("nama", is("dummy 1"))))
                .andExpect(model().attribute("store", hasProperty("keterangan", is("Keterangan 1"))));
